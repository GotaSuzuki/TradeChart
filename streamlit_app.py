...
from app.market_data import compute_rsi as compute_price_rsi, download_price_history
...
def main() -> None:
    st.set_page_config(page_title="SEC 10-K Dashboard", layout="wide")
    config, fetcher = _init_services()
    st.title("SEC 10-K Dashboard")

    st.markdown(
        "SEC EDGAR から10-Kを取得し、主要指標と前年比を可視化します。"
    )

    with st.sidebar:
        st.header("パラメータ")
        ticker_input = st.text_input("ティッカーシンボル", value="NVDA")
    ...
    view = st.radio("表示ビュー", ("ファンダメンタル", "テクニカル", "アラート"), horizontal=True)
    ...
    if fetch_clicked:
        ...
        with st.spinner("SECから10-Kを取得中..."):
            try:
                filings = fetcher.fetch_recent_filings(ticker, years=years)
            except Exception as exc:  # pragma: no cover - UIエラー通知
                st.error(f"取得に失敗しました: {exc}")
                return

        if not filings:
            st.warning("指定ティッカーの10-Kが見つかりませんでした。")
            return

        metrics = extract_financials(filings)
        if not metrics:
            st.warning("財務データを抽出できませんでした。")
            return
...
def render_metric_panels(df, cagr_map):
    ...
    for idx, (metric, label) in enumerate(METRIC_LABELS.items()):
        ...
        metric_df["value_m"] = (
            metric_df["value"] / scale if scale else metric_df["value"]
        )
...
def render_technical_section(ticker: str, period_label: str):
    ...
    melted = recent.melt(id_vars="Date", value_vars=display_cols, var_name="Series", value_name="Price")
    ...
    fig = px.line(
        ...
        y="Price",
        color="Series",
        labels={"Date": "日付", "Price": "価格 (USD)", "Series": "系列"},
    )
    ...
    for trace in fig.data:
        if trace.name.upper() == "CLOSE":
            ...
            trace.hovertemplate = "%{x|%Y-%m-%d}<br>Close: %{y:.2f} USD" + (
    ...
    if latest_price is not None and latest_rsi is not None:
            ...
            rows.append(
                {
                    "項目": label,
                    "RSI": target,
                    "目安価格": _estimate_price_for_rsi(
                        latest_price, latest_rsi, target
                    ),
                    "highlight": False,
                }
            )
        ...
        df["価格表記"] = df["目安価格"].map(
            lambda x: f"{x:,.2f} USD" if pd.notna(x) else "-"
        )
        display_df = df[["項目", "RSI表記", "価格表記"]]
        styled = display_df.style.apply(
            lambda row: [
                "background-color: rgba(255, 220, 0, 0.3)" if row.name == row.name and df.iloc[row.name]["highlight"] else ""
                for _ in row
            ],
            axis=1,
        ).set_table_styles(
...
def _determine_scale(value):
    ...
    try:
        ...
        return 1, ""
    if magnitude != magnitude:  # NaN check
        return 1, ""
    thresholds = [
        (1_000_000_000_000, "兆"),
        (1_000_000_000, "B"),
...
def _apply_cross_shading(fig, price_df):
    ...
    for start, end, state in segments:
        ...
        if start_ts >= end_ts:
            continue
        color = "rgba(255, 99, 132, 0.15)" if state == 1 else "rgba(100, 149, 237, 0.15)"
        fig.add_vrect(
            x0=start_ts,
            x1=end_ts,
            fillcolor=color,
...
def _render_latest_price(price_df: pd.DataFrame) -> None:
...
def render_alerts_page() -> None:
    ...
    for ticker in df["ticker"].unique():
        ...
        if price_df.empty:
            current_data.append(
                {"ticker": ticker, "current_price": None, "current_rsi": None}
            )
            continue
        price_df = _append_rsi(price_df)
        ...
        current_data.append(
            {
                ...
                "current_rsi": price_df.dropna(subset=["RSI"]).iloc[-1].get("RSI")
                if not price_df.dropna(subset=["RSI"]).empty
                else None,
            }
        )
